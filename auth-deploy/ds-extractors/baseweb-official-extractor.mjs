...
const dsDir = path.join(__dirname, '..', 'ds_packages', 'baseweb');
...
async function fetchJson(url) {
  const r = await fetch(url, { headers: { 'User-Agent': 'comtra-baseweb-extractor/1.0', Accept: 'application/vnd.github+json' } });
...
async function run() {
  ...
  const repo = await fetchRepoTree('uber', 'baseweb');

  const tokenFiles = repo.tree
    .filter((x) => x.type === 'blob' && /\.(ts|tsx|js|json)$/.test(x.path))
    .filter((x) => /(theme|token|colors|typography|sizing|borders)/i.test(x.path))
    .map((x) => x.path)
    .slice(0, 200);

  const componentFiles = repo.tree
    .filter((x) => x.type === 'blob' && x.path.startsWith('src/'))
    .filter((x) => /\.(tsx|ts|js)$/.test(x.path))
    .map((x) => x.path)
    .slice(0, 250);

  tokens.official_import = tokens.official_import || {};
  tokens.official_import.baseweb = {
    extracted_at: new Date().toISOString(),
    source_repo: 'uber/baseweb',
    ref: repo.ref,
  ...
  components.official_import.baseweb = {
    extracted_at: new Date().toISOString(),
    source_repo: 'uber/baseweb',
    ref: repo.ref,
  ...
  const report = {
    ds_id: 'baseweb',
    extracted_at: new Date().toISOString(),
    extractor: 'baseweb-official-extractor',
    elapsed_ms: Date.now() - start,
    source: { repo: 'uber/baseweb', ref: repo.ref, token_source_count: tokenFiles.length, component_source_count: componentFiles.length },
  };
...
run().catch((err) => {
  console.error('[baseweb-official-extractor] failed', err?.message || err);
  process.exit(1);
});
