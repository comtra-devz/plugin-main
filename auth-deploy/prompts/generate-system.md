...
- **INSTANCE_COMPONENT**: the plugin only finds components **already in the user’s Figma file** (any page) or imports by **published Figma component key**. DS manifest ids (e.g. `baseweb/button`) are **not** Figma layer names — they usually resolve to placeholders. **Prefer `CREATE_RECT` + `CREATE_TEXT`** for buttons, fields, and links unless you know a matching component name exists in the file.
