...
- **actions**: for v1 "create" mode, include at least one CREATE_FRAME for structure **and** you MUST add **at least one** `CREATE_TEXT` or `CREATE_RECT` with `parentId: "root"` (or omit `parentId` so it defaults to root). Strong layouts also add more visible primitives (`CREATE_TEXT`, `CREATE_RECT`, and/or `INSTANCE_COMPONENT` when allowed) under `root` or child frame `ref`. Empty shells are invalid: e.g. login → headline text, logo/brand area (frame or rect), primary button (rect or component), social/links row (text or rects). Use `CREATE_TEXT` with `characters` for labels; `CREATE_RECT` for buttons/fields when no matching component exists in context.
- **INSTANCE_COMPONENT**: allowed **only** when `metadata.ds_source` is **custom** (or equivalent “current file” scope) **and** the DS CONTEXT INDEX lists a matching component. When using a **bundled public DS** (Material, Ant, Carbon, etc.), you **must not** output `INSTANCE_COMPONENT` — use only `CREATE_FRAME`, `CREATE_TEXT`, and `CREATE_RECT` plus DS **token** variable paths from the package. For custom DS, the plugin resolves components **already in the user’s Figma file** or by **published Figma component key**. DS manifest ids (e.g. `baseweb/button`) are **not** Figma layer names — they usually resolve to placeholders. **Prefer `CREATE_RECT` + `CREATE_TEXT`** when public DS or when no matching component exists in the file.
