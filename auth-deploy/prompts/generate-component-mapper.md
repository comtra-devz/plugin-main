...
3. **Variables only** for spacing and fills: for custom/file DS, use only variable names listed in `[DS CONTEXT INDEX].variable_names`. Do not invent generic refs like `spacing/md` or `color/surface/default`; if no exact listed variable fits, omit that property. No raw hex or px in padding/fills.
