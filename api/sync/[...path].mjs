/**
 * Proxy for /api/sync/* when auth.comtra.dev is served by the root Vercel project
 * instead of the auth-deploy project. The real sync API lives on the auth backend.
 */
...
export default async function handler(req, res) {
  ...
  try {
    const u = new URL(req.url || '/api/sync', 'https://n.local');
    ...
    console.error('[sync proxy]', e);
    res.statusCode = 502;
    res.setHeader('Content-Type', 'application/json; charset=utf-8');
    return res.end(JSON.stringify({ error: 'Sync upstream unavailable' }));
  }
}
