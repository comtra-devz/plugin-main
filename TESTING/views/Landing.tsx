...
export const Landing: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  useEffect(() => {
    ...
    const type = () => {
      if (step === 0) {
        if (charIndex < t1.length) {
          setText1(prev => t1.slice(0, charIndex + 1));
          charIndex++;
          setTimeout(type, 100);
        } else {
          step = 1;
        ...
        if (charIndex < t2.length) {
          setText2(prev => t2.slice(0, charIndex + 1));
          charIndex++;
          setTimeout(type, 100);
        } else {
          step = 2;
        ...
        if (charIndex < t3.length) {
          setText3(prev => t3.slice(0, charIndex + 1));
          charIndex++;
          setTimeout(type, 100);
        }
      }
  ...
  return (
    <div className="w-full min-h-screen bg-white text-black relative font-sans">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-black text-white border-b-2 border-white p-4 flex justify-between items-center h-16">
        <h1 className="text-2xl font-black uppercase tracking-tighter leading-[0.9] hover:text-[#ff90e8] transition-colors cursor-pointer">
          Comtra <span className="text-xs bg-yellow-400 text-black px-1 ml-1 align-top">TEST</span>
        </h1>
        <button 
          onClick={onBack} 
      ...
      <section className="relative pt-32 pb-0 px-6 min-h-[110vh] flex flex-col items-center justify-start text-center overflow-hidden bg-[#fdfdfd]">
        ...
        >
          ...
          <h2 
            className="text-7xl md:text-9xl font-black uppercase leading-[0.85] mb-8 tracking-tighter mix-blend-multiply relative z-10 font-['Tiny5'] min-h-[3em]"
            aria-label="Stop Wrestling Figma"
          >
            <span aria-hidden="true">{text1}</span><br/>
            <span aria-hidden="true" className="text-transparent bg-clip-text bg-gradient-to-r from-[#ff90e8] to-[#ffc900]">{text2}</span><br/>
            <span aria-hidden="true">{text3}</span>
            <span className="animate-pulse text-[#ff90e8]" aria-hidden="true">_</span>
          </h2>
          ...
          <p className="text-lg font-medium text-gray-800 mb-10 max-w-xl mx-auto leading-relaxed bg-white/60 backdrop-blur-sm p-2 rounded border border-transparent">
            The first tool that truly audits your design system. 
            Detect drift, respect tokens, and sync components directly to Storybook using semantic AI.
          </p>
        </div>
        ...
        <div className="z-20 relative w-full max-w-4xl mt-12 mb-20 perspective-1000">
           <div className="bg-white border-4 border-black shadow-[16px_16px_0_0_#000] rounded-t-lg overflow-hidden transform transition-transform hover:scale-[1.01] duration-500">
              ...
              <div className="p-16 md:p-24 bg-white flex flex-col items-center justify-center relative overflow-hidden min-h-[400px]">
                 ...
                 <button 
                    onClick={onBack}
                    className="relative z-10 bg-black text-white border-2 border-transparent text-xl md:text-xl font-black uppercase px-8 py-6 hover:bg-[#ff90e8] hover:text-black hover:border-black hover:shadow-[8px_8px_0_0_#000] transition-all active:translate-x-1 active:translate-y-1 active:shadow-none transform -rotate-2 hover:rotate-0 flex items-center gap-3 group"
                 >
                   <svg width="24" height="24" viewBox="0 0 38 57" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M9.5 57C14.7467 57 19 52.7467 19 47.5V38H9.5C4.25329 38 0 42.2533 0 47.5C0 52.7467 4.25329 57 9.5 57Z" fill="#0ACF83" className="transition-colors group-hover:fill-white"/>
                      <path d="M0 28.5C0 23.2533 4.25329 19 9.5 19H19V38H9.5C4.25329 38 0 33.7467 0 28.5Z" fill="#A259FF" className="transition-colors group-hover:fill-white"/>
                      <path d="M0 9.5C0 4.25329 4.25329 0 9.5 0H19V19H9.5C4.25329 19 0 14.7467 0 9.5Z" fill="#F24E1E" className="transition-colors group-hover:fill-white"/>
                      <path d="M19 0H28.5C33.7467 0 38 4.25329 38 9.5C38 14.7467 33.7467 19 28.5 19H19V0Z" fill="#FF7262" className="transition-colors group-hover:fill-white"/>
                      <path d="M38 28.5C38 33.7467 33.7467 38 28.5 38H19V19H28.5C33.7467 19 38 23.2533 38 28.5Z" fill="#1ABCFE" className="transition-colors group-hover:fill-white"/>
                   </svg>
                   <span>Try for free with Figma</span>
                 </button>
              </div>
           </div>
        </div>
      ...
      <div className="overflow-hidden w-full">
        <div className="bg-[#ff90e8] border-y-2 border-black py-4 text-black rotate-1 scale-105 z-30 relative shadow-[0_-4px_10px_rgba(0,0,0,0.1)]">
            <div className="animate-marquee whitespace-nowrap">
            <MarqueeItem text="Smart Routing" />
            <MarqueeItem text="Drift Detection" />
            <MarqueeItem text="Semantic HTML" />
      ...
      {/* Stacking Workflow Section - RESTORED */}
      <section className="bg-[#fdfdfd] py-32 px-6 relative border-b-2 border-black">
        <div className="max-w-3xl mx-auto">
            <h2 className="text-5xl font-black uppercase mb-20 text-center font-['Tiny5']">How it works</h2>
            
            {/* Card 1 */}
            <div className="sticky top-24 z-10 bg-white border-4 border-black p-12 shadow-[8px_8px_0_0_#000] mb-12 flex flex-col gap-4 transform -rotate-1 transition-transform hover:scale-[1.02]">
                <div className="flex justify-between items-start">
                    <span className="text-6xl font-black text-gray-200">01</span>
                    <div className="bg-black text-white px-3 py-1 font-bold uppercase text-xs">Audit</div>
                </div>
                <h3 className="text-4xl font-black uppercase">Scan your System</h3>
                <p className="text-lg font-medium text-gray-600 max-w-md">
                    Comtra analyzes every layer, text node, and component. It detects hardcoded values, accessibility failures, and broken links instantly.
                </p>
            </div>

            {/* Card 2 */}
            <div className="sticky top-32 z-20 bg-[#ffc900] border-4 border-black p-12 shadow-[8px_8px_0_0_#000] mb-12 flex flex-col gap-4 transform rotate-1 transition-transform hover:scale-[1.02]">
                <div className="flex justify-between items-start">
                    <span className="text-6xl font-black text-black/10">02</span>
                    <div className="bg-black text-white px-3 py-1 font-bold uppercase text-xs">Fix</div>
                </div>
                <h3 className="text-4xl font-black uppercase">Auto-Correction</h3>
                <p className="text-lg font-medium text-black max-w-md">
                    Don't waste hours renaming layers. Use our AI to apply bulk fixes, standardize naming conventions, and link tokens automatically.
                </p>
            </div>

            {/* Card 3 */}
            <div className="sticky top-40 z-30 bg-[#ff90e8] border-4 border-black p-12 shadow-[8px_8px_0_0_#000] flex flex-col gap-4 transform -rotate-1 transition-transform hover:scale-[1.02]">
                <div className="flex justify-between items-start">
                    <span className="text-6xl font-black text-black/10">03</span>
                    <div className="bg-black text-white px-3 py-1 font-bold uppercase text-xs">Generate</div>
                </div>
                <h3 className="text-4xl font-black uppercase">Deploy to Code</h3>
                <p className="text-lg font-medium text-black max-w-md">
                    Export production-ready React, Vue, or Liquid code. We map your design tokens to CSS variables so developers don't have to guess.
                </p>
            </div>
        </div>
      </section>
      ...
      <footer className="bg-black text-white py-20 px-6 text-center border-t-4 border-white z-20 relative">
        <h4 className="font-black uppercase text-4xl md:text-6xl mb-8 tracking-tighter font-['Tiny5']">Make peace with your developers</h4>
        ...
        <div className="flex flex-wrap justify-center gap-6 mb-8 text-xs font-bold uppercase tracking-wider text-gray-400">
           ...
           <button onClick={() => setShowPrivacyModal(true)} className="hover:text-white hover:underline uppercase">Privacy e Policy</button>
           <span className="text-gray-700">•</span>
      ...
      {/* Privacy Modal */}
      {showPrivacyModal && (
        <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-6 overflow-y-auto">
           <div className={`${BRUTAL.card} bg-white max-w-lg w-full relative`}>
